...
const AccountDetails: React.FC<AccountDetailProps> = ({
  ...
  return (
    <div className={classes.root}>
      ...
      <button className={classes.button} onClick={() => setModalOpen(2)}>
        {' '}
        <Typography variant="button"> Delete Account </Typography>{' '}
      </button>
      <button className={classes.buttonFilled} onClick={() => setModalOpen(3)}>
        {' '}
      ...
      <ConfirmationModal
        open={modalOpen === 2}
        onCancel={() => setModalOpen(0)}
        onContinue={handleDeleteAccount}
        actionDescription="delete this account"
      />
      <ConfirmationModal
        open={modalOpen === 3}
        onCancel={() => setModalOpen(0)}
        onContinue={resetPassword}
        actionDescription="reset this account's password"
      />
    </div>
  );
};
